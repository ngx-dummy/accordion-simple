...
export const getSvg = (file: string, sanitaizer: DomSanitizer) => sanitizeRes('data:image/svg+xml;base64,' + btoa(file), sanitaizer);
export const getPng = (file: string, sanitaizer: DomSanitizer) => sanitizeRes(URL.createObjectURL(pngBase64ToBlob(file)), sanitaizer);
