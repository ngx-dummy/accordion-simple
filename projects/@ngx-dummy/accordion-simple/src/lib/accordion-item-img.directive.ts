...
export class AccordionItemImgDirective
	...
	ngAfterContentChecked() {
		...
		if (this.isLogoImg()) {
			...
			return this.setItemHeaderImgSageSrc(
				this.readElDataSet('src') || this._baseLogoImg
			);
	...
	onLoad = ({ target, ...rest }: { target: HTMLImageElement; rest: any[] }) =>
		this.setLoadSuccess(target);
	onError = ({
		target,
		...rest
	}: {
	...
	private doOpenCloseSrcRecheck = () => {
		this.zone.runOutsideAngular(() => {
			this._src = this.nativeImgEl.classList.contains('open')
				? this._closeSign
				: this._openSign;
		});
	};
	private setItemHeaderImgSrc = (
		imgRef: HTMLImageElement,
		src: string | SafeResourceUrl
	) => this.renderer.setAttribute(imgRef, 'src', src as string);
	...
	private isImgOpenClose = () =>
		!!this._closeSign &&
		!!(this._closeSign as string).length &&
		!!this._openSign &&
		!!(this._openSign as string).length;
	private isLogoImg = () =>
		this.nativeImgEl?.classList.contains(ImgClasses.LOGO);
	private isCloserImg = () =>
		this.nativeImgEl?.classList.contains(ImgClasses.ENDIMG);
