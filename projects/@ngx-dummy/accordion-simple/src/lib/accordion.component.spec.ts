...
import {
	TestHostModule,
	TestHostComponent,
} from '../helpers/test-host.component';
...
describe('An Accordion component', () => {
	...
	describe('single component', () => {
		beforeEach(async () => {
			...
			accordCmpFixture = testingBed.createComponent<AccordionComponent>(
				AccordionComponent
			);
		...
		beforeEach(() => {
			...
			accordCmp.accordionStyling.itemStyling = [
				sampleItemStyling,
				additionItemStyling,
			];
		...
		it('should contain [ngxdAccordion] directive as attribute of element with .accordion class', () => {
			const cmp = accordCmpDebugEl.query(By.directive(AccordionDirective))
		...
		it('should contain ngxd-accordion-item element as the first child', (cb) => {
			...
			const getAccordItemEl: () => HTMLElement = () =>
				cmp.querySelectorAll('ngxd-accordion-item').item(0) as HTMLElement;
			const getAccordItemElHeader = () =>
				getAccordItemEl().querySelector('.accord-item__header') as HTMLElement;
			const getAccordItemElBody = () =>
				getAccordItemEl().querySelector('.accord-item__body') as HTMLElement;

			expect(Array.from(getAccordItemElBody().classList)).toContain('closed');
			expect(getAccordItemEl().nodeName.toLowerCase()).toEqual(
				'ngxd-accordion-item'
			);
	...
	describe('several Accordions in a surface', () => {
		...
		beforeEach(async () => {
			...
			hostCmpFixture = testingBed.createComponent<TestHostComponent>(
				TestHostComponent
			);
			accordCmp = accordCmpFixture.componentInstance;
		...
		beforeEach(() => {
			...
			(hostDebEl.query(By.css('#container'))
				.nativeElement as HTMLElement).insertAdjacentElement(
				'afterbegin',
		...
		it('should have 2 Accordions', () => {
			const cmp = hostCmpFixture.debugElement.query(
				By.directive(AccordionComponent)
			);
			const accordDirective = hostCmpFixture.debugElement.query(
				By.directive(AccordionDirective)
			);
			expect(cmp).toBeTruthy();
