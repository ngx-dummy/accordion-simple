...
describe('AppComponent', () => {
	...
	it('should get accordion', () => {
		const getChildAccordion: () => DebugElement = () =>
			appCmpFixture.debugElement.query(By.css('ngxd-accordion:first-of-type'));
		const getChildAccordionCmpInstance = () =>
			getChildAccordion().componentInstance;
		...
		expect(
			(getChildAccordionCmpInstance() as AccordionComponent).name.toLowerCase()
		).toContain('accordion');
		expect(
			getChildAccordion().nativeElement.querySelectorAll('ngxd-accordion-item')
				.length
		).toBeGreaterThan(1);
	});
});
